...
void o_programme() {
	...
	cout <<
		"ТЕКУЩЕЙ РЕЛИЗ:\n"
		"v.1.42 (15.03.2020)\n"
		"- Добавлены настройки пользовательского теста;\n"
		"- Добавлены настройки цвета консоли;\n"
		...
		"- Исправлена ошибка, связанная с вылет  программы  при попытке  загрузить\n"
		"в текстовые файлы не равное количество слов;\n"
		"- В итоговой  статистике  прохождения  теста  добавлена  отметка о верных\n"
		"и не верных ответах;\n\n"

		"v.1.40 (09.03.2020)\n"
		"- Добавлен  новый  вид  теста  «Пользовательский тест»  -  Тест,  который\n"
		"составляется из пары слов, заданных пользователем;\n"
		"- При  заполнении  пользовательского теста,  текстовые  файлы  в  которые\n"
		"сохраняются слова открываются автоматически;\n"
		"- В программе  реализована  возможность  сохранить  пары  слов, з аданные\n"
		"пользователем, для использования их при  следующем  включении  программы,\n"
		"а также стереть их из программы;\n\n"

		"v.1.33 (05.03.2020)\n"
		"- Исправлена ошибка,  связанная  с  завершением  теста  при  вводе  слов,\n"
		"оканчивающихся на букву \"t\"\n"
		"- Изменен метод принимаемых в качестве ответа  слов.  В  качестве  ответа\n"
		"принимается только одно слово.В обучающей программе слова требующие более\n"
		"детальный перевод сообщается  в виде  ключевого  слова  с  комментарием в\n"
		"круглых скобках;\n\n"
...
void nastroika() {
	...
	cout << "2 - Задать цвет консоли:\n\n";

	cout << " : НАСТРОЙКА СЛУЧАЙНОГО ТЕСТА :\n";
	cout << "3 - Количество вопросов для случайного теста: \n";
	cout << "4 - Количество пар слов для случайного теста: \n\n";

	cout << " : НАСТРОЙКА ТЕСТОВ ОБУЧАЮЩЕЙ ПРОГРАММЫ :\n";
	cout << "5 - Количество вопросов для обучающих тестов: \n";
	cout << "6 - Количество пар слов для обучающих тестов: \n\n";

	cout << " : НАСТРОЙКА ЭКЗАМЕНАЦИОННЫХ ТЕСТОВ :\n";
	cout << "7 - Количество вопросов для итоговых тестов: \n\n";

	cout << " : НАСТРОЙКА ПОЛЬЗОВАТЕЛЬСКОГО ТЕСТА :\n";
	cout << "8 - Количество вопросов для пользовтельского теста: \n";
	cout << "9 - Подсказки в пользовтельском тесте: \n\n";
	...
	if (z == 1) {
		...
		z = getValue();
		if (z == 1) {
			system("color 9D");// Фон голубоуй, шрифт рпурный
		}
		if (z == 2) {
			system("color A5");// Желтыйб , пурпуный
		}
		if (z == 3) {
			system("color E0");// Ораньжевый черный
		}
		if (z == 4) {
			system("color F0");
		}
		...
		int par;
		par = getValue();
		if (par == 1 || par == 10) s_test.par = 10;
		else if (par == 2 || par == 20) s_test.par = 20;
		...
		i_test.qwest = getValue();
		cout << "Установленно впоросов: " << i_test.qwest << endl;
	}

	else if (z == 8) {
		cout << "Укажите количество вопросов для пользовтельского теста:\n"
			"Сечас вопросов: " << per_test.qwest << endl;
		per_test.qwest = getValue();
		cout << "Установленно впоросов: " << per_test.qwest << endl;
	}
	else if (z == 9) {
		cout << "Подсказки для пользовтельского теста:\n"
			"Сечас подсказки: ";
		if (per_test.Podskazka == 1) cout << "Включены\n";
		else if (per_test.Podskazka == 0) cout << "Выключены\n";
		cout << "1 - Включить\n" "2 - Выключить\n";
		z = getValue();
		if (z == 1) per_test.Podskazka = 1;
		else if (z == 2) per_test.Podskazka = 0;
		cout << "Сечас подсказки: ";
		if (per_test.Podskazka == 1) cout << "Включены\n";
		else if (per_test.Podskazka == 0) cout << "Выключены\n";
	}
...
string bloc_prost_rus(int x) {
	...
	case(6): clovo = "артикль \n(неопределённый артикль, для сущ. начинающегося с согласного звука)"; break;
	...
	case(32): clovo = "артикль \n(неопределённый артикль, для сущ. начинающегося с гласного звука)"; break;
	case(33): clovo = "хотеть (проявлять волю)"; break;
	case(34): clovo = "мой"; break;
	case(35): clovo = "один"; break;
	case(36): clovo = "все"; break;
	case(37): clovo = "вероятность \n(модальный глагол, выражающий желание, вероятность)"; break;
...
string bloc_glagol_rus(int x) {
	...
	case(8): clovo = "модуль \n (модальный глагол, выражающий желание, вероятность, просьбу)"; break;
	...
	case(24): clovo = "Модуль \n(модальный глагол, выражающий вероятность, разрешение)"; break;
	case(25): clovo = "Модуль \n(модальный глагол, выражающий долженствование)"; break;
	...
	case(43): clovo = "возможно \n(может быть, прош. форма глагола ман)"; break;
	...
	case(55): clovo = "Модуль \n(модальный глагол, выражающий сильную степень долженствования)"; break;
